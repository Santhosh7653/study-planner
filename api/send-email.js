...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Manual parse fallback if body comes as string
    ...
    const { type, title, taskTitle, deadline, priority, notes, oldPriority, oldDate } = body;
    const taskName = title || taskTitle;

    if (!type || !taskName) {
      console.error('[send-email] Missing fields. Received:', JSON.stringify(body));
